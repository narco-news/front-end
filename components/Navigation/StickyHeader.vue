<template>
	<div class="sticky">
		<header class="flex flex-wrap text-white text-center text-xs md:text-sm">
			<div id="blank" class="w-1/4 select-none">
				{{ $t('message.date') }}
			</div>
			<div id="language" class="w-1/4">
				<LanguageBox />
			</div>
			<div id="tips" class="w-1/4 flex gap-2 md:gap-4 justify-center">
				<n-link class="flex" to="/wire">
					<div class="mr-2">
						<svg
					...
					<div class="hidden md:inline text-xs uppercase whitespace-no-wrap">
						{{ $t('message.newswire') }}
					</div>
				</n-link>
				<a class="flex" href="#">
					...
					<div class="hidden md:inline text-xs uppercase whitespace-no-wrap">
						{{ $t('message.discussionboard') }}
					</div>
				</a>
			</div>
			<div id="icons" class="w-1/4 flex justify-end pr-4">
				<a
...
export default {
	...
	i18n: {
		messages: {
			en: {
				message: {
					date: dayjs().locale('en').format('MMMM D, YYYY'),
					newswire: 'Newswire',
			...
			es: {
				message: {
					date: dayjs().locale('es').format('MMMM D, YYYY'),
